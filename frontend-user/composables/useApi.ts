...
function getImageUrl(image: string | null, apiBase: string): string | null {
  ...
  if (image.startsWith('/uploads')) {
    const serverBase = apiBase.replace(/\/api(\/public)?$/, '')
...
export const useApi = () => {
  ...
  const publicApiBase = `${apiBase}/public`

  const getAllPosts = async () => {
    try {
      const response = await $fetch<PaginatedPosts>(`${publicApiBase}/posts?limit=100`)
  ...
  const getAllPostsPaginated = async (options?: { page?: number, limit?: number, search?: string }) => {
    try {
      ...
      const url = queryString ? `${publicApiBase}/posts?${queryString}` : `${publicApiBase}/posts`
  ...
  const getPostBySlug = async (slug: string) => {
    try {
      const data = await $fetch<Post>(`${publicApiBase}/posts/slug/${encodeURIComponent(slug)}`)
  ...
  const getPostsByCategory = async (categorySlug: string) => {
    try {
      const data = await $fetch<Post[]>(`${publicApiBase}/posts/category/${encodeURIComponent(categorySlug)}`)
  ...
  const searchPosts = async (query: string) => {
    try {
      const data = await $fetch<Post[]>(`${publicApiBase}/posts/search?q=${encodeURIComponent(query)}`)
      return (data || []).map(p => transformPost(p, apiBase))
    } catch (e) {
  ...
  const getRelatedPosts = async (postId: number, limit = 3) => {
    try {
      const data = await $fetch<Post[]>(`${publicApiBase}/posts/${postId}/related?limit=${limit}`)
      return (data || []).map(p => transformPost(p, apiBase))
    } catch (e) {
  ...
  const incrementViews = async (postId: number) => {
    try {
      await $fetch(`${publicApiBase}/posts/${postId}/view`, { method: 'POST' })
    } catch (e) {
  ...
  const incrementLikes = async (postId: number) => {
    try {
      await $fetch(`${publicApiBase}/posts/${postId}/like`, { method: 'POST' })
      return true
    } catch (e) {
